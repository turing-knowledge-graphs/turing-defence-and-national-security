# Event on Knowledge Graphs and ontologies for Defence and National Security
Turing Event on the funded KG projects by the Grand Challenge on Defence and National Security

**11:00-11:15** Welcome

**11:15-12:45** Overview of the funded projects: Phase 1 + Phase 2 planning (Part 1)
- 2 x 45 min (allowing at least 15 min for questions).
- Summary of the research, achievements, surpising results, potential connection with other projects.

**12:45-13:45** Lunch and networking

**13:45-14:45** Keynote/Invited speaker (45 min + questions).  
- [Alexandra Luck](https://www.linkedin.com/in/alexandra-luck-9804a45/), Programme Directorof the National Digital Twin Programme (NDTP)

**14:45-16:15** Overview of the funded projects: Phase 1 + Phase 2 planning (Part 2)
- 2 x 45 min (allowing at least 15 min for questions).
- Summary of the research, achievements, surpising results, potential connection with other projects.

**16:15-17:15** Networking activity + coffee
- Whiteboarding session to map intersections between the four funded projects.
- Summary into a 3-minute elevator pitch for a potential joint EPSRC/Turing bid.

**17:15-** Closing + social activity


**Projects:**
- CLSKG: A Common Logic Substrate for Multi-LLM Integration with Uncertainty & Ontology-Based Safety Checks
- Uncertainty-Aware Entity Resolution for Robust Knowledge Graphs
- PICASO: Probabilistic Conceptual Spaces Sensemaking under Uncertainty
- [GUARD: Ensuring Interoperable and Trustworthy Knowledge Graphs for Defence and National Security AI](https://ernestojimenezruiz.github.io/projects/guard/)
  - PI: [Ernesto Jimenez-Ruiz](https://ernestojimenezruiz.github.io/)
